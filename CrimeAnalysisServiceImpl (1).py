...
from exception.myExceptions  import IncidentNumberNotFound, CaseNumberNotFound
from tabulate import tabulate

class CrimeAnalysisServiceImpl(DBConnUtil, ICrimeAnalysisService):
    def createIncident(self, Incidents):
        try:
            self.cursor.execute(
                "INSERT INTO Incidents (IncidentID, IncidentType, IncidentDate, Loc_Longitude, Loc_Latitude, Description, Status, VictimID, SuspectID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (Incidents.IncidentID, Incidents.IncidentType, Incidents.IncidentDate, Incidents.Loc_Longitude,
                 Incidents.Loc_Latitude, Incidents.Description, Incidents.Status, Incidents.VictimID,
                 Incidents.SuspectID),
            )
            self.conn.commit()
    ...
    def updateIncidentStatus(self, Status, IncidentID):
        try:
            ...
            if len(Incident) > 0:
                ...
                raise IncidentNumberNotFound(IncidentID)
        except Exception as e:
            print(e)
    ...
    def generateIncidentReport(self, Reports):
        try:
            self.cursor.execute(
                ...
                (Reports.ReportID, Reports.IncidentID, Reports.ReportingOfficer, Reports.ReportDate,
                 Reports.ReportDetails, Reports.Status),
            )
            self.conn.commit()
    ...
    def updateCaseDetails(self, CaseID, Details):
        try:
            ...
            if len(Case) > 0:
                ...
                raise CaseNumberNotFound(CaseID)
        except Exception as e:
            print(e)
